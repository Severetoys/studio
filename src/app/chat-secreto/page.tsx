...
import { ArrowLeft, Send, Video, User, Loader2, PhoneOff, Languages, MapPin } from 'lucide-react';
...
export default function ChatSecretoPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, async (querySnapshot) => {
            ...
            for (const doc of querySnapshot.docs) {
                ...
                // Se a mensagem for do admin e eu sou o usuário, traduz para o meu idioma
                if (data.senderId === 'admin' && data.originalText && data.text) {
                    msgs.push({ id: doc.id, ...data, text: data.text });
                } 
                // Se a mensagem for minha, não preciso traduzir, mas o admin verá o originalText
                else if (data.senderId === currentUser.uid) {
                     msgs.push({ id: doc.id, ...data });
                }
                // Se a msg for do admin mas não tem originalText (msg antiga)
                else if (data.senderId === 'admin') {
                     msgs.push({ id: doc.id, ...data });
                }
                
                // Este caso não deve acontecer se a lógica estiver correta, mas é um fallback
                else {
                    msgs.push({ id: doc.id, ...data });
                }
            }
    ...
    const handleSendMessage = async (text: string, isLocation = false) => {
        ...
        try {
            // Detect user language and save it to the chat document
            ...
            // Translate message to Portuguese for the admin
            const translated = await translateText({ text: trimmedMessage, targetLanguage: 'pt' });

            await addDoc(messagesCollection, {
                text: translated.translatedText, // PT for admin
                originalText: trimmedMessage, // Original for user and context
    ...
    return (
        <main className="flex flex-1 w-full flex-col items-center justify-center p-0 md:p-4 bg-transparent md:bg-background">
            <Card className="w-full h-full md:h-[85vh] max-w-md flex flex-col animate-in fade-in-0 zoom-in-95 duration-500 shadow-neon-red-strong border-primary/50 bg-black/90 backdrop-blur-xl md:rounded-2xl">
